...
export const labsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getlabs: builder.query({
      query: (searchText) => ({
        url: `lab/list-all-labs?search=${searchText}`,
        method: "GET",
      }),
      providesTags: ["labs"],
    }),
    ...
    // ✅ Update labs Status by ID
    updatelabsStatus: builder.mutation({
      query: ({ id, body }) => ({
        url: `/lab/update-lab-status/${id}`,
        method: "PUT",
        body,
      }),
      invalidatesTags: ["labs"],
